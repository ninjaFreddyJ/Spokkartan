-- ============================================================
-- 👻 SPÖKKARTAN — KÖR HELA DENNA FIL I SUPABASE
--
-- SÅ HÄR GÖR DU (1 gång):
--   1. Öppna supabase.com -> ditt projekt
--   2. Vänster meny: 'SQL Editor'  ->  '+ New query'
--   3. Markera ALLT i denna fil, klistra in, klicka 'Run' (▶)
--
-- Detta är ofarligt att köra om (idempotent). Den:
--   • lägger till alla nya platser (de som inte redan finns)
--   • berikar befintliga med bättre text (utan att röra bild/koordinat)
--   • tar bort dubbletter
--   • skriver längst ner ut hur många platser du har nu
-- ============================================================
...
-- ============================================================
-- ✅ KLART. Resultatet nedan visar facit:
-- ============================================================
SELECT
  (SELECT count(*) FROM places)                                   AS total_platser_nu,
  (SELECT count(*) FROM places WHERE source LIKE 'web_%')         AS nya_denna_session,
  (SELECT count(*) FROM (
      SELECT lower(name) n FROM places GROUP BY 1 HAVING count(*) > 1
  ) d)                                                            AS kvarvarande_dubbletter;
